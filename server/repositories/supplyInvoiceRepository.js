...
export const SupplyInvoiceRepository = {
  ...
  insertItems(invoiceId, items) {
    const stmt = db.prepare(
      `INSERT INTO supply_invoice_items (invoice_id, product_id, quantity, unit_cost, line_total)
       VALUES (?, ?, ?, ?, ?)`
    )
    for (const item of items) {
      stmt.run(invoiceId, item.product_id, item.quantity, item.unit_cost, item.line_total)
    }
  },
}